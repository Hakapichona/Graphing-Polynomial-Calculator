...
class Recta:
    ...
    def recta_paralela(self, punto):
        m = (self.punto2.y - self.punto1.y) / (self.punto2.x - self.punto1.x)
        b = punto.y - m * punto.x
        return f"Ecuación de la recta paralela: y = {m}x + {b}"
...
x3 = float(input("Ingrese la coordenada x del punto por el que pasa la recta paralela: "))
y3 = float(input("Ingrese la coordenada y del punto por el que pasa la recta paralela: "))

punto_paralelo = Punto2D(x3, y3)

ecuacion = recta.ecuacion_recta()
ecuacion_paralela = recta.recta_paralela(punto_paralelo)
...
print(f"Punto por el que pasa la recta paralela: ({x3}, {y3})")
print(ecuacion_paralela)
